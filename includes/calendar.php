...
*/
add_shortcode('thq_connect_calendar','thq_connect_calendar_function'); //Calendar shortcode
wp_enqueue_style('thq-connect-calendar-styles', plugin_dir_url(__FILE__).'../css/calendar.css','','2.2.4');

/* Function called by shortcode */
function thq_connect_calendar_function($atts) {
	$content = new thq_connect_calendar($atts);
} //end function

class thq_connect_calendar {
	public $param;
	private $currentYear;
    ...
    public function __construct($param = '') {
			...
			/* set year */		
			$this->currentYear = !empty($_GET['cal-y']) ? $_GET['cal-y'] : date("Y",time());
			/* set month */
			$this->currentMonth = !empty($_GET['cal-m']) ? $_GET['cal-m'] : date("m",time());
			/* set day */
			...
			/* Calendar Settings */
			$this->calendarOptions = get_option('thq_connect_calendar_settings');
			...
			$days_in_lastmonth = cal_days_in_month(0,$this->lastMonth, $this->lastYear);
			...
			/* Days last month */
			$day_count = 1;
			$title = $days_in_lastmonth - $this->offset +1;
			while ($this->offset > 0) {
			...
			while ($day_num <= $this->daysInMonth) {
				...
				if ($day_num == $this->currentDay && $this->currentMonth == date('m',time()) && $this->currentYear == date('Y',time())) { $content .= ' class="active"'; }
				...
				if(!empty($this->tasks[$thisDay])){
					foreach ($this->tasks[$thisDay] as $event) {
						...
						//$content .= '<span class="calendar-event-time">'.$event['time'].'</span>'; //No time in tasks
						$content .= '<span class="calendar-event-title">'.$event['summary'].'</span>';
						$content .= '</a>';
					} //end foreach
				} //end if
			...
			$content .= '<span id="toggle-calendar-event" style="background-color: '.$this->calendarOptions['event_colour'].'">Events</span>';
			$content .= '<span  id="toggle-calendar-meeting" style="background-color: '.$this->calendarOptions['meeting_colour'].'">Meetings</span>';
	...
	public function _getEvents() {
		...
		if((empty($this->calendarOptions['event_flag']) || $this->calendarOptions['event_flag'] == 'enabled') && !in_array("event",$this->notDisplayed)) {
			$events = $get->request('events',array('start_at'=>$monthStartDay,'end_at'=>$monthEndingDay));
			$allEvents = array();
			if(!empty($events) && $events != '400') {
				date_default_timezone_set($this->timezone);
				foreach($events as $event) {
					$eventDay = date("Y-n-j",strtotime($event['start_at']));
	...
	public function _getMeetings() {
		...
		if((empty($this->calendarOptions['meeting_flag']) || $this->calendarOptions['meeting_flag'] == 'enabled') && !in_array("meeting",$this->notDisplayed)) {  //if meeting_flag is enabled
			$meetings = $get->request('meetings');
			$calenderMeetings = array();
			if(!empty($meetings) && $meetings != '400') {
				date_default_timezone_set($this->timezone);
				foreach($meetings as $meeting) {
					$meetingDay = date("Y-n-j",strtotime($meeting['date_at']));  //Note: date_at which is different to the dev.tidyhq.com docs
	...
	public function _getTasks() {
		...
		if((empty($this->calendarOptions['task_flag']) || $this->calendarOptions['task_flag'] == 'enabled') && !in_array("task",$this->notDisplayed)) {  //if task_flag is enabled
			$tasks = $get->request('tasks',array('start_at'=>$monthStartDay,'end_at'=>$monthEndingDay));
			$calenderTasks = array();
			if(!empty($tasks) && $tasks != '400') {
				date_default_timezone_set($this->timezone);
				foreach($tasks as $task) {
					$taskDay = date("Y-n-j",strtotime($task['due_date']));
					$taskTime = '';
	...
	public function _getPublicEvents() {
		if((empty($this->calendarOptions['event_flag']) || $this->calendarOptions['event_flag'] == 'enabled') && !in_array("event",$this->notDisplayed)) {  //if event_flag is enabled
			$ical = new thq_connect_ical();
			$lines = $ical->load( file_get_contents( "https://".$this->thqSettings['domain_prefix'].".tidyhq.com/public/schedule/events.ics" ) );
			$events = $lines['VEVENT'];
			...
			if(!empty($events)) {
				date_default_timezone_set($this->timezone);
				foreach ($events as $event) {
					$eventDay = date("Y-n-j",strtotime($event['DTSTART']));
	...
	public function _getPublicMeetings() {
		if((empty($this->calendarOptions['meeting_flag']) || $this->calendarOptions['meeting_flag'] == 'enabled') && !in_array("meeting",$this->notDisplayed)) {  //if meeting_flag is enabled
			$ical = new thq_connect_ical();
			$lines = $ical->load( file_get_contents( "https://".$this->thqSettings['domain_prefix'].".tidyhq.com/public/schedule/meetings.ics" ) );
